defmodule VideoCommentatorWeb.AuthTest do
  use VideoCommentatorWeb.ConnCase
  alias VideoCommentatorWeb.Auth

  setup %{conn: conn} do
    conn =
      conn
      |> bypass_through(VideoCommentatorWeb.Router, :browser)
      |> get("/")
  ...
  test "authenticate_user continues when the current_user exists", %{conn: conn} do
    conn =
      conn
      |> assign(:current_user, %VideoCommentator.Accounts.User{})
  ...
  test "login puts the user in the session", %{conn: conn} do
    login_conn =
      conn
      |> Auth.login(%VideoCommentator.Accounts.User{id: 123})
      |> send_resp(:ok, "")
